...
void ofxSvgLoader::validateXmlSvgRoot( ofXml& aRootSvgNode ) {
	...
	if(aRootSvgNode) {
		...
		if( !wattr || !hattr ) {
			restore_to_viewbox = { 1.0f, 1.0f }; // apply viewbox at 100%
		} else {
			restore_to_viewbox = { 1.0f, 1.0f };
			if (ofStringEndsWith(wattr.getValue(),"%")) restore_to_viewbox.value().x = wattr.getFloatValue()/100.0;
			if (ofStringEndsWith(hattr.getValue(),"%")) restore_to_viewbox.value().y = hattr.getFloatValue()/100.0;
		}
